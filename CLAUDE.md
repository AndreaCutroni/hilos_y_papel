...
│   └── quaderni/      # generated by scripts/quaderni-webp.py, one folder per notebook
├── components/
...
│   ├── quaderni.ts    # the catalogue: one entry per notebook
│   └── sketchbook.ts  # the /chi-sono plates
├── lib/               # motion, hooks, handwriting, quaderniFoto (the photo globs)
...
One entry per notebook in `content/quaderni.ts`, in catalogue order. The photos
start as originals in `references/images/quaderni/<slug>/` (gitignored);
`python scripts/quaderni-webp.py` writes each one 800px and 1600px wide into
`src/assets/quaderni/<slug>/`, and `lib/quaderniFoto.ts` finds them by glob.
The script mirrors the originals: re-run it after adding, renaming or removing
a photo, and it deletes the WebPs whose original is gone.

- **Adding a notebook**: a folder of photos, the script, then an entry.
  `tipologia`, `taglia`, `orientamento` and `genere` are the owner's answers —
  never guess them from a photo. `copertina` names the cover photo; `taglio`
  moves the square crop in the grid when the centre misses the notebook. In
  dev, the console warns about a notebook with no photos or a missing cover.
...
- Filters live in the query string, so a filtered grid can be shared and
  ...
  `/quaderni/:slug` contains each photo and never crops it. The second photo
  fades in on hover only where a real hover exists.
